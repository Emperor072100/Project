...
def generar_historial_inicial():
    ...
    try:
        ...
        campañas_sin_historial = (
            db.query(Campaña)
            .filter(~Campaña.id.in_(db.query(HistorialCampaña.campaña_id).distinct()))
            .all()
        )
        ...
        db.commit()
        print(
            f"✅ Historial inicial generado para {len(campañas_sin_historial)} campañas"
        )
