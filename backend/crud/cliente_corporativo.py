...
def get_cliente_corporativo(
    ...
    return (
        db.query(ClienteCorporativo).filter(ClienteCorporativo.id == cliente_id).first()
    )
...
def get_clientes_corporativos_by_sector(
    db: Session, sector: str
) -> List[ClienteCorporativo]:
    """Obtener clientes corporativos por sector"""
    return (
        db.query(ClienteCorporativo).filter(ClienteCorporativo.sector == sector).all()
    )


def create_cliente_corporativo(
    db: Session, cliente: ClienteCorporativoCreate
) -> ClienteCorporativo:
...
def update_cliente_corporativo(
    db: Session, cliente_id: int, cliente_update: ClienteCorporativoUpdate
) -> Optional[ClienteCorporativo]:
...
def delete_cliente_corporativo(db: Session, cliente_id: int) -> bool:
