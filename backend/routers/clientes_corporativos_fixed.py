...
router = APIRouter(
    prefix="/clientes-corporativos",
    tags=["clientes-corporativos"]
)
...
def listar_clientes_corporativos(
    ...
    clientes = crud_cliente_corp.get_clientes_corporativos(
...
def obtener_cliente_corporativo(
    cliente_id: int,
    db: Session = Depends(get_db)
):
    """Obtener un cliente corporativo por ID"""
    cliente = crud_cliente_corp.get_cliente_corporativo(
...
def crear_cliente_corporativo(
    ...
    return crud_cliente_corp.create_cliente_corporativo(
...
def eliminar_cliente_corporativo(
    cliente_id: int,
    db: Session = Depends(get_db)
):
    """Eliminar un cliente corporativo"""
    success = crud_cliente_corp.delete_cliente_corporativo(
...
def obtener_clientes_por_sector(
    sector: str,
    db: Session = Depends(get_db)
):
    """Obtener clientes corporativos por sector"""
    clientes = crud_cliente_corp.get_clientes_corporativos_by_sector(
        db, sector=sector
    )
    return clientes
