...
def crear_tarea(
    ...
    proyecto = db.query(modelos_proyecto.Proyecto).filter_by(id=tarea.proyecto_id).first()
    if not proyecto or (usuario.rol != "admin" and proyecto.responsable_id != usuario.id):
        raise HTTPException(status_code=403, detail="No autorizado para agregar tareas a este proyecto")
...
def actualizar_tarea(
    ...
    proyecto = db.query(modelos_proyecto.Proyecto).filter_by(id=db_tarea.proyecto_id).first()
